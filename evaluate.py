...
def main(
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        # 假设 sourceTensor 是一个张量
        sourceTensor = torch.tensor([[8241, 3782]], dtype=torch.float32)
        # 使用 clone() 和 detach() 复制张量
        copied_tensor = sourceTensor.clone().detach()
        # 计算 logits
        logits = copied_tensor.softmax(dim=-1)
